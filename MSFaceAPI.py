...
def add_person_face(personId,image_url):
    ...
    try:
        conn = httplib.HTTPSConnection('westcentralus.dev.cognitive.microsoft.com')
        '''conn.request("POST", "/face/v1.0/persongroups/%s/persons/%s/persistedFaces?" %(personGroupId,personId), body, headers)'''
        conn.request("POST", "/face/v1.0/persongroups/users/persons/%s/persistedFaces?" % personId, body, headers)
        conn = httplib.HTTPSConnection('westcentralus.api.cognitive.microsoft.com')
        response = conn.getresponse()
        data = response.read()
        data = json.loads(data)
